import pytest, ctypes, subprocess, os
from ctypes import c_uint8 as uint8_t

S_BOX = (uint8_t * 256)(*[
     
    ])

R_CON = (uint8_t * 11)(*([0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C]))

keyLen = 16
totalKeyLen = 176

# I pulled these test vectors from the AES standard
test_vectors = [
    ("2b7e151628aed2a6abf7158809cf4f3c", "6bc1bee22e409f96e93d7e117393172a", "3ad77bb40d7a3660a89ecaf32466ef97"),
    ("2b7e151628aed2a6abf7158809cf4f3c", "ae2d8a571e03ac9c9eb76fac45af8e51", "f5d3d58503b9699de785895a96fdbaaf"),
    ("2b7e151628aed2a6abf7158809cf4f3c", "30c81c46a35ce411e5fbc1191a0a52ef", "43b1cd7f598ece23881b00e3ed030688"),
    ("2b7e151628aed2a6abf7158809cf4f3c", "f69f2445df4f9b17ad2b417be66c3710", "7b0c785e27e8ad3f8223207104725dd4")
]

lib = ctypes.CDLL("build/libmyProgram_lib.so")

@pytest.mark.parametrize("key,plain,cipher", test_vectors)
def test_Encrypt(key, plain, cipher):
    text = (uint8_t * 16)()
    key_array = (uint8_t * 16)()
    expandedKeys = (uint8_t * 176)()

    # Convert the hex strings to byte arrays
    key_array[:] = bytes.fromhex(key)
    text[:] = bytes.fromhex(plain)

    lib.KeyExpansion(bytes.fromhex(key), expandedKeys, S_BOX, R_CON, keyLen, totalKeyLen)

    lib.Encrypt(text, expandedKeys, S_BOX)

    # Convert the result to a hex string
    result = (''.join([hex(byte)[2:].zfill(2) for byte in text])).upper()

    assert result == cipher.upper(), f"For key: {key}, plain: {plain}, expected: {cipher}, but got: {result}"
    

